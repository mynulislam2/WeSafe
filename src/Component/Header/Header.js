...
const Header = ({ setSwitcheduser, Switcheduser, Activeusers, setActiveusers }) => {
    ...
    useEffect(() => {
        const getUser = async () => {
            ...
            querySnapshot.forEach(async (docs) => {
                ...
                const querySnapshotForProfile = await getDoc(doc(db, `Users/${user?.uid}/ChildList/${child}/data`, "personal_information"));
                const ProfileData = querySnapshotForProfile.data()
                setActiveusers(ProfileData)
                setActiveusersProfile(ProfileData?.profilePicUrl)
                setSwitcheduser(child)
                ...
                if (res === true) {
                    ...
                    deleteDoc(DeletedocRef)
                        .then(async() => {
                            ...
                            setUsers(newData)
                            setSwitcheduser(docId[0])
                            const querySnapshot1 = await getDoc(doc(db, `Users/${user?.uid}/ChildList/${docId[0]}/data`, "personal_information"));
                            const data = querySnapshot1.data()
                            setActiveusers(data)
                            setActiveusersProfile(data.profilePicUrl)
                            localStorage.setItem("activeUser", JSON.stringify(docId[0]))
                            forceUpdate()
                            setDoc(doc(db, `Users/${user?.uid}/ChildList/${docId[0]}`), {
                                ActiveStatus: true,
                                uid: user?.uid
                            });
                ...
                <div className="dropdown dropdown-end">
                    <label for="my-modal-6" className="btn modal-button btn-ghost btn-circle avatar">
                        <div className="w-10 rounded-full" onClick={() => setStart(true)}>
                            <img src={ActiveusersProfile ? ActiveusersProfile : user?.photoURL} alt="" />
                        </div>
                    </label>
                </div>
            </div>
